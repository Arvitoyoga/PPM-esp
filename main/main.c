...
void manual_command() {
    uint32_t now = xTaskGetTickCount() * portTICK_PERIOD_MS;
        ...
        if (prevR && !touchR) readyR = true;
        if (prevL && !touchL) readyL = true;
...
if (countL > 0 &&
    ...
    ESP_LOGI(TAG, "Left tap timeout");
        ...
        if (!syncWaiting &&
            ...
            if (countR == 1) {ESP_LOGI(TAG, "3"); manualCMD = 3; execute_command(manualCMD);}
            else if (countR == 2) {ESP_LOGI(TAG, "7"); manualCMD = 7; execute_command(manualCMD);}
            else if (countR == 3) {ESP_LOGI(TAG, "8"); manualCMD = 8; execute_command(manualCMD);}
        ...
        if (!syncWaiting &&
            ...
            if (countL == 1) {ESP_LOGI(TAG, "2"); manualCMD = 2; execute_command(manualCMD);}
            else if (countL == 2) {ESP_LOGI(TAG, "9"); manualCMD = 9; execute_command(manualCMD);}
            else if (countL == 3) {ESP_LOGI(TAG, "6"); manualCMD = 6; execute_command(manualCMD);}
...
void oled_task() {
    while(1) {
        ...
        oled_set_cursor(0,2);
        oled_print("Mode       : ");
        if(ManualMode) oled_print("Manual");
        else oled_print("Voice ");

        oled_set_cursor(0,3);
        oled_print("Payload Seq: ");
        if(PaySeq==0) oled_print("0");
        else if(PaySeq==1) oled_print("1");
        else if(PaySeq==2) oled_print("2");
        else if(PaySeq==3) oled_print("3");

        oled_set_cursor(0,4);
        oled_print("Drone Pos  : ");
        if(DronePos==0) oled_print("Front");
        else if(DronePos==1) oled_print("Right");
        else if(DronePos==2) oled_print("Front");
        else if(DronePos==3) oled_print("Left ");
        ...
        oled_print("Front: ");
        if(DropperPosA==0) oled_print("Right");
        else if(DropperPosA==1) oled_print("Mid  ");
        else if(DropperPosA==2) oled_print("Left ");

        oled_set_cursor(15,7);
        oled_print("Rear : ");
        if(DropperPosB==0) oled_print("Right");
        else if(DropperPosB==1) oled_print("Mid  ");
        else if(DropperPosB==2) oled_print("Left ");
...
ESP_ERROR_CHECK(uart_set_pin(uart_num, -1, UART_RX_PIN, -1, -1));
    ...
    xTaskCreatePinnedToCore(rmt_task, "rmt", 4096, NULL, 5, NULL, 1);
    xTaskCreatePinnedToCore(uart_task, "uart", 4096, NULL, 4, NULL, 0);
    xTaskCreatePinnedToCore(oled_task, "oled", 4096, NULL, 6, NULL,1);
    xTaskCreatePinnedToCore(buzzer_task, "buzzer", 4096, NULL, 7, NULL,1);
